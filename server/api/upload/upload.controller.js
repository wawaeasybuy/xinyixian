...
exports.upload=function (req,res){
	var data = _.pick(req.body, 'type'), 
        ...
        fs.rename(file[0].path,__dirname+"/../../../client/assets/upload/upload_images/"+file[0].name,function(err){
			 if(err){
			    console.log("重命名失败！");
			 }else{
			    console.log("重命名成功！");
...
exports.uploadToLib=function (req,res){
	var data = _.pick(req.body, 'type'), 
        ...
        // console.log(__dirname+"/images/"+file[0].name);
        // console.log(__dirname+"/../../../client/assets/upload_iamges/");
        fs.rename(file[0].path,__dirname+"/../../../client/assets/upload/images_lib/"+file[0].name,function(err){
			 if(err){
			    console.log("重命名失败！");
			 }else{
			    console.log("重命名成功！");
